"""EpiDeep weekly grid search driver for CG-Mamba (v2.1.7-A++ M2.3 expansion).

Faithful EpiDeep implementation (Adhikari et al. 2019, KDD).
Mirrors scripts/run_dlinear_weekly.py protocol (Pattern A):
...
Spec (PLAN §7.1 EpiDeep baseline, Option α: 12 cfg modern cluster):
  GRID  = d_emb × encoder_hidden × lr = 2 × 2 × 3 = 12 configs
  FIXED = seq_len=104, pred_len=4, enc_in=6, decoder_hidden=128,
          alignment_weight=0.1, dropout=0.1,
          batch_size=16, epochs=100, patience=20

Training loss (with auxiliary alignment):
  L = MSE(forecast, y) + α · MSE(encoder(x), bank[season_id])    where α=0.1

Reference:
  Adhikari, B., Xu, X., Ramakrishnan, N., & Prakash, B.A. (2019). KDD 2019.

Run:
  CUDA_VISIBLE_DEVICES=0 python scripts/run_epideep_weekly.py --mode smoke
  CUDA_VISIBLE_DEVICES=0 python scripts/run_epideep_weekly.py --mode auto
"""
...
from baselines.epideep import EpiDeepForecaster, build_epideep_loaders    # type: ignore  # noqa: E402
...
FIXED = {
    ...
    "enc_in": 6,
    "decoder_hidden": 128,
    "alignment_weight": 0.0,            # v2.1.7-A++: paper-faithful Adhikari 2019 (α=0)
    "target_only": False,               # v2.1.7-A++: multivariate V=6 default
    "dropout": 0.1,
    "batch_size": 16,
    "epochs": 100,
    "patience": 20,
...
WANDB_BASE_TAGS = ["epideep", "weekly", "baseline", "v2.1.7-A++"]
...
def init_wandb(enabled, mode, cfg, seed, run_name, extra_tags=None):
    ...
    run = wandb.init(
        ...
        group=f"epideep_{mode}_v2.1.7-A++",
        name=run_name, tags=tags,
        config={**cfg, "seed": seed, "phase": "M2.3_EpiDeep"},
        reinit=True,
    )
...
def config_dirname(cfg: dict) -> str:
    return f"de{cfg['d_emb']}_eh{cfg['encoder_hidden']}_lr{cfg['lr']:.0e}"


def build_model(cfg: dict) -> EpiDeepForecaster:
...
def train_one_run(
    ...
    train_loader, val_loader, meta = build_epideep_loaders(
    ...
    for epoch in range(1, epochs + 1):
        model.train()
        tr_fc_losses, tr_align_losses = [], []
        for x, s_id, y in train_loader:
            x, s_id, y = x.to(device), s_id.to(device), y.to(device)
            pred = model(x)
            fc_loss = criterion(pred, y)
            align_loss = model.compute_alignment_loss(x, s_id)
            loss = fc_loss + alpha * align_loss

            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            tr_fc_losses.append(float(fc_loss.item()))
            tr_align_losses.append(float(align_loss.item()))

        model.eval()
        all_pred, all_true = [], []
        with torch.no_grad():
            for x, s_id, y in val_loader:
        ...
        train_fc = float(np.mean(tr_fc_losses))
        train_align = float(np.mean(tr_align_losses))

        if wandb_run is not None:
            wandb_run.log({
                "epoch": epoch,
                "train_fc_loss": train_fc, "train_align_loss": train_align,
        ...
        if val_mae_h1 < best_val_mae_h1:
            ...
            best_epoch = epoch
            patience_counter = 0
            torch.save(model.state_dict(), out_dir / "epideep_best.pt")
        else:
            patience_counter += 1
...
def run_smoke(csv_path, norm_path, device, out_root, wandb_enabled=True):
    cfg = {"d_emb": 64, "encoder_hidden": 64, "lr": 1e-3, **FIXED}
    smoke_dir = out_root / "epideep_smoke"
...
def run_grid(csv_path, norm_path, device, out_root, wandb_enabled=True):
    grid_root = out_root / "epideep_grid"
    ...
    for i, cfg in enumerate(cells, 1):
        ...
        rows.append({"config_dir": cname,
                     "d_emb": cfg["d_emb"], "encoder_hidden": cfg["encoder_hidden"], "lr": cfg["lr"],
...
def run_final(csv_path, norm_path, device, out_root, config_json, wandb_enabled=True):
    ...
    final_root = out_root / "epideep_final" / config_dirname(cfg)
...
def main():
    ap = argparse.ArgumentParser(description="EpiDeep weekly baseline (v2.1.7-A++)")
    ...
    print(f"[EpiDeep v2.1.7-A++] mode={args.mode}, device={args.device}, wandb={'ON' if wandb_enabled else 'OFF'}")
