"""Vanilla Mamba weekly grid search driver for CG-Mamba (v2.1.7-A++ M2.3 expansion).

Mirrors scripts/run_dlinear_weekly.py (Pattern A — identical protocol for fair
comparison):
...
Spec (PLAN §7.4 A1 ablation, Pattern A independent HPO):
  GRID  = d_model × n_layers × lr = 2 × 2 × 3 = 12 configs
  FIXED = seq_len=104, pred_len=4, enc_in=6, d_state=16, dt_rank=16,
          expand=2, dropout=0.0, batch_size=32, epochs=200, patience=20

Architecture: CGMambaBackbone (gate-less M1.2 vanilla), wrapped with multi-horizon
Linear head. Same SSM core as CG-Mamba's CGMambaEncoder; only difference is
absence of per-layer context gating via gate_proj.

Run:
  CUDA_VISIBLE_DEVICES=0 python scripts/run_vanilla_mamba_weekly.py --mode smoke
  CUDA_VISIBLE_DEVICES=0 python scripts/run_vanilla_mamba_weekly.py --mode auto
"""
...
from baselines.vanilla_mamba import VanillaMambaForecaster   # type: ignore  # noqa: E402
...
WANDB_BASE_TAGS = ["vanilla_mamba", "weekly", "baseline", "v2.1.7-A++", "A1_ablation"]
...
def init_wandb(enabled, mode, cfg, seed, run_name, extra_tags=None):
    ...
    run = wandb.init(
        ...
        group=f"vanilla_mamba_{mode}_v2.1.7-A++",
        name=run_name, tags=tags,
        config={**cfg, "seed": seed, "phase": "M2.3_VanillaMamba"},
        reinit=True,
    )
...
def config_dirname(cfg: dict) -> str:
    return f"d{cfg['d_model']}_nl{cfg['n_layers']}_lr{cfg['lr']:.0e}"


def build_model(cfg: dict) -> VanillaMambaForecaster:
...
def train_one_run(
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_mae_h1 < best_val_mae_h1:
            ...
            best_epoch = epoch
            patience_counter = 0
            torch.save(model.state_dict(), out_dir / "vanilla_mamba_best.pt")
        else:
            patience_counter += 1
...
def run_smoke(csv_path, norm_path, device, out_root, wandb_enabled=True):
    cfg = {"d_model": 64, "n_layers": 2, "lr": 5e-4, **FIXED}
    smoke_dir = out_root / "vanilla_mamba_smoke"
...
def run_grid(csv_path, norm_path, device, out_root, wandb_enabled=True):
    grid_root = out_root / "vanilla_mamba_grid"
    ...
    for i, cfg in enumerate(cells, 1):
        ...
        rows.append({"config_dir": cname,
                     "d_model": cfg["d_model"], "n_layers": cfg["n_layers"], "lr": cfg["lr"],
...
def run_final(csv_path, norm_path, device, out_root, config_json, wandb_enabled=True):
    ...
    final_root = out_root / "vanilla_mamba_final" / config_dirname(cfg)
...
def main():
    ap = argparse.ArgumentParser(description="Vanilla Mamba weekly baseline (v2.1.7-A++ A1 ablation)")
    ...
    print(f"[Vanilla Mamba v2.1.7-A++] mode={args.mode}, device={args.device}, wandb={'ON' if wandb_enabled else 'OFF'}")
