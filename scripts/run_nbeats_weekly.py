"""N-BEATS weekly grid search driver for CG-Mamba (v2.1.7-A++ M2.3 expansion).
...
Spec (PLAN §7.1 N-BEATS baseline, Option α: 12 cfg modern-architecture cluster):
  GRID  = hidden × n_blocks × lr = 2 × 2 × 3 = 12 configs
  FIXED = seq_len=104, pred_len=4, enc_in=6, n_layers=4, batch_size=16,
          epochs=100, patience=20

Reference:
  Oreshkin et al. 2020 ICLR — Generic mode, single stack of N blocks.

Run:
  CUDA_VISIBLE_DEVICES=0 python scripts/run_nbeats_weekly.py --mode smoke
  CUDA_VISIBLE_DEVICES=0 python scripts/run_nbeats_weekly.py --mode auto
"""
...
from baselines.nbeats import NBeatsForecaster              # type: ignore  # noqa: E402
from baselines.lstm import build_lstm_loaders               # type: ignore  # noqa: E402


GRID = {
    "n_blocks": [3, 6, 12, 24, 30],     # v2.1.7-A++ capacity sweep incl. paper-faithful Generic-30
    "lr":       [5e-4, 1e-3],
}                                         # 5 × 2 = 10 cfg
FIXED = {
    "seq_len": 104,
    "pred_len": 4,
    "enc_in": 6,
    "hidden": 512,                        # paper standard (fixed across sweep)
    "n_layers": 4,                        # paper standard
...
WANDB_BASE_TAGS = ["nbeats", "weekly", "baseline", "v2.1.7-A++"]
...
def init_wandb(enabled, mode, cfg, seed, run_name, extra_tags=None):
    ...
    run = wandb.init(
        ...
        group=f"nbeats_{mode}_v2.1.7-A++",
        name=run_name, tags=tags,
        config={**cfg, "seed": seed, "phase": "M2.3_NBEATS"},
        reinit=True,
    )
...
def config_dirname(cfg: dict) -> str:
    suffix = "_tgt" if cfg.get("target_only") else ""
    return f"nb{cfg['n_blocks']}_h{cfg['hidden']}_lr{cfg['lr']:.0e}{suffix}"


def build_model(cfg: dict) -> NBeatsForecaster:
...
def train_one_run(
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_mae_h1 < best_val_mae_h1:
            ...
            best_epoch = epoch
            patience_counter = 0
            torch.save(model.state_dict(), out_dir / "nbeats_best.pt")
        else:
            patience_counter += 1
...
def run_smoke(csv_path, norm_path, device, out_root, wandb_enabled=True):
    cfg = {"n_blocks": 3, "lr": 1e-3, **FIXED}
    smoke_dir = out_root / "nbeats_smoke"
...
def run_grid(csv_path, norm_path, device, out_root, wandb_enabled=True):
    grid_root = out_root / "nbeats_grid"
    ...
    for i, cfg in enumerate(cells, 1):
        ...
        rows.append({"config_dir": cname,
                     "n_blocks": cfg["n_blocks"], "hidden": cfg["hidden"], "lr": cfg["lr"],
                     "target_only": cfg.get("target_only", False),
...
def run_final(csv_path, norm_path, device, out_root, config_json, wandb_enabled=True):
    ...
    final_root = out_root / "nbeats_final" / config_dirname(cfg)
...
def main():
    ap = argparse.ArgumentParser(description="N-BEATS weekly baseline (v2.1.7-A++)")
    ...
    print(f"[N-BEATS v2.1.7-A++] mode={args.mode}, device={args.device}, wandb={'ON' if wandb_enabled else 'OFF'}")
