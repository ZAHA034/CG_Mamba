"""TimesNet weekly grid search driver for CG-Mamba (v2.1.7-A++ M2.3 expansion).
...
Spec (PLAN §7.1 TimesNet baseline, Option α: 12 cfg modern-architecture cluster):
  GRID  = d_model × e_layers × lr = 3 × 2 × 2 = 12 configs
  FIXED = seq_len=104, pred_len=4, enc_in=6, batch_size=16,
          epochs=100, patience=20, top_k=5, num_kernels=6, dropout=0.1

Output:
  runs/timesnet_grid/d{d_model}_el{e_layers}_lr{lr}/
    ├── timesnet_best.pt
    └── results.json
  runs/timesnet_grid/grid_summary.csv
  runs/timesnet_final/{config_dirname}/seed{S}/...

Run:
  CUDA_VISIBLE_DEVICES=0 python scripts/run_timesnet_weekly.py --mode smoke
  CUDA_VISIBLE_DEVICES=0 python scripts/run_timesnet_weekly.py --mode auto
"""
...
from baselines.timesnet import TimesNetForecaster        # type: ignore  # noqa: E402
...
WANDB_BASE_TAGS = ["timesnet", "weekly", "baseline", "v2.1.7-A++"]
...
def init_wandb(enabled, mode, cfg, seed, run_name, extra_tags=None):
    ...
    run = wandb.init(
        ...
        group=f"timesnet_{mode}_v2.1.7-A++",
        name=run_name, tags=tags,
        config={**cfg, "seed": seed, "phase": "M2.3_TimesNet"},
        reinit=True,
    )
...
def grid_iter():
    ...
    for combo in itertools.product(*[GRID[k] for k in keys]):
        ...
        cfg.update(FIXED)
        cfg["d_ff"] = cfg["d_model"] * 2     # d_ff = 2 × d_model (thuml convention)
        yield cfg


def config_dirname(cfg: dict) -> str:
    return f"d{cfg['d_model']}_el{cfg['e_layers']}_lr{cfg['lr']:.0e}"


def build_model(cfg: dict) -> TimesNetForecaster:
...
def train_one_run(
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_mae_h1 < best_val_mae_h1:
            ...
            best_epoch = epoch
            patience_counter = 0
            torch.save(model.state_dict(), out_dir / "timesnet_best.pt")
        else:
            patience_counter += 1
...
def run_smoke(csv_path, norm_path, device, out_root, wandb_enabled=True):
    cfg = {"d_model": 64, "e_layers": 2, "lr": 5e-4, **FIXED}
    cfg["d_ff"] = cfg["d_model"] * 2
    smoke_dir = out_root / "timesnet_smoke"
...
def run_grid(csv_path, norm_path, device, out_root, wandb_enabled=True):
    grid_root = out_root / "timesnet_grid"
    ...
    for i, cfg in enumerate(cells, 1):
        ...
        rows.append({"config_dir": cname,
                     "d_model": cfg["d_model"], "e_layers": cfg["e_layers"], "lr": cfg["lr"],
...
def run_final(csv_path, norm_path, device, out_root, config_json, wandb_enabled=True):
    ...
    final_root = out_root / "timesnet_final" / config_dirname(cfg)
...
def main():
    ap = argparse.ArgumentParser(description="TimesNet weekly baseline (v2.1.7-A++)")
    ...
    print(f"[TimesNet v2.1.7-A++] mode={args.mode}, device={args.device}, wandb={'ON' if wandb_enabled else 'OFF'}")
