"""PatchTST weekly grid search driver for CG-Mamba (v2.1.6 baseline expansion).
...
Spec (PLAN §7.1 PatchTST baseline, dropout NOT swept per user decision):
  GRID  = patch_len × d_model × lr = 3×2×2 = 12 configs
  FIXED = seq_len=104, pred_len=4, enc_in=6, n_heads=4,
          e_layers=2, d_ff=2*d_model, dropout=0.1,
          batch_size=16, epochs=100, patience=20
  stride = patch_len // 2 (50% overlap, thuml official)

Output:
  runs/patchtst_grid/pl{patch_len}_dm{d_model}_lr{lr}/
    ├── patchtst_best.pt
    └── results.json
  runs/patchtst_grid/grid_summary.csv
  runs/patchtst_final/{config_dirname}/seed{S}/...

W&B:
  entity  = hjs40111-personal
  project = cg-mamba-jbhi
  group   = patchtst_{mode}_v2.1.6
  tags    = ["patchtst", "weekly", "baseline", "v2.1.6", mode]

Run:
  CUDA_VISIBLE_DEVICES=0 python scripts/run_patchtst_weekly.py --mode smoke
  CUDA_VISIBLE_DEVICES=0 python scripts/run_patchtst_weekly.py --mode grid
  CUDA_VISIBLE_DEVICES=0 python scripts/run_patchtst_weekly.py --mode final \
      --config-json runs/patchtst_grid/<best>/results.json
"""
...
from baselines.patchtst import PatchTSTForecaster  # type: ignore  # noqa: E402
...
FIXED = {
    ...
    "stride_ratio": 0.5,         # stride = max(1, int(patch_len * stride_ratio))
    "dropout": 0.1,              # paper default; not swept (user decision)
    "batch_size": 16,
    "epochs": 100,
    "patience": 20,
}
...
WANDB_BASE_TAGS = ["patchtst", "weekly", "baseline", "v2.1.6"]
...
def init_wandb(enabled, mode, cfg, seed, run_name, extra_tags=None):
    ...
    run = wandb.init(
        ...
        group=f"patchtst_{mode}_v2.1.6",
        name=run_name,
        tags=tags,
        config={**cfg, "seed": seed, "phase": "M2.6_PatchTST"},
        reinit=True,
    )
...
def config_dirname(cfg: dict) -> str:
    return f"pl{cfg['patch_len']}_dm{cfg['d_model']}_lr{cfg['lr']:.0e}"


def build_model(cfg: dict) -> PatchTSTForecaster:
...
def train_one_run(
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_mae_h1 < best_val_mae_h1:
            ...
            best_epoch = epoch
            patience_counter = 0
            torch.save(model.state_dict(), out_dir / "patchtst_best.pt")
        else:
            patience_counter += 1
    ...
    if wandb_run is not None:
        ...
        try:
            artifact = wandb.Artifact(f"patchtst_best_{wandb_run_name}", type="model")
            artifact.add_file(str(out_dir / "patchtst_best.pt"))
...
def run_smoke(csv_path, norm_path, device, out_root, wandb_enabled=True):
    cfg = {"patch_len": 16, "d_model": 128, "lr": 5e-4, **FIXED}
    smoke_dir = out_root / "patchtst_smoke"
...
def run_grid(csv_path, norm_path, device, out_root, wandb_enabled=True):
    grid_root = out_root / "patchtst_grid"
...
def run_final(csv_path, norm_path, device, out_root, config_json, wandb_enabled=True):
    ...
    final_root = out_root / "patchtst_final" / config_dirname(cfg)
...
# ---------------------------------------------------------------------------
def main():
    ap = argparse.ArgumentParser(description="PatchTST weekly baseline (v2.1.6)")
    ...
    print(f"[PatchTST v2.1.6] mode={args.mode}, device={args.device}")
